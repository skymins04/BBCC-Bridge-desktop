          ...
          loadDcCon = function () {
            ...
            dcConScript.onload = function () {
              if (dcConsData.length === 0) {
                ...
                keywordsWithIndex.sort(function (a, b) {
                ...
                applyDcCon = function (message, data) {
                  for (const idx in keywordsWithIndex) {
                    ...
                    if (message.indexOf(`~${keyword}`) !== -1) {
                      ...
                      if (
                        dccon.url !== undefined &&
                        typeof dccon.url === "string" &&
                        dccon.url.startsWith("http")
                      ) {
                        message = message
                          .split(`~${keyword}`)
                          .join(`<img class="dccon" src="${dccon.url}" />`);
                      } else if (
                        dccon.uri !== undefined &&
                        typeof dccon.uri === "string" &&
                        dccon.uri.startsWith("http")
                      ) {
                        message = message
                          .split(`~${keyword}`)
                          .join(`<img class="dccon" src="${dccon.uri}" />`);
                      } else {
                        message = message
                          .split(`~${keyword}`)
                          .join(
                            `<img class="dccon" src="${
                              configData.dcConsURI + dcConsSubURI + dccon.name
                            }" />`
                          );
                      }
                    }
